...
def max_vacation_days(flights, days)
    memo = Array.new(flights.length){Array.new(days[0].length){-Float::INFINITY}}
    
    return dfs(flights, days, 0, 0, memo)
end

def dfs(flights, days, cur_city, weekno, memo)
    if weekno == days[0].length
        return 0
    end
    if memo[cur_city][weekno] != -Float::INFINITY
        return memo[cur_city][weekno]
    end
    maxvac = 0
    (0...flights.length).each do |i|
        if flights[cur_city][i] == 1 || i == cur_city
            vac = days[i][weekno] + dfs(flights, days, i, weekno + 1, memo)
            maxvac = [maxvac, vac].max
        end
    end
